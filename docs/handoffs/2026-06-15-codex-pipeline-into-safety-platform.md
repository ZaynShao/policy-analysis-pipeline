...
| 谁 | 做什么 |
|---|---|
| **Codex(可直接做)** | Phase A 全部:在 monorepo 本地 clone 准备 `services/policy-pipeline/` 内容、改 compose 卷路径、补 OPERATIONS、本地 pytest + docker build 验证、push 分支、开 MR(草稿)。Phase C 的 README 指针 commit。 |
...
- **不写生产 vault / 不碰 `0_raw/`**。
- **MR 草稿**:Codex push 分支 + 开 **draft MR**,合并由用户/平台团队决定(进的是公司仓)。
- 失败回滚锚保留:`/root/policy-pipeline-src` 不删。

---

## Phase A · 代码落仓 + 开 MR(Codex,在 monorepo 本地 clone)

源 = pipeline 仓 **干净 main**;目标 = monorepo `services/policy-pipeline/`。
- pipeline 本地仓:`/Users/shaoziyuan/dev/政策分析-pipeline`
- monorepo 本地 clone:`/Users/shaoziyuan/Documents/战略大盘/safety-platform`

### Task A1:取干净 main + 建 monorepo 分支
...
- [ ] **Step 2**:monorepo 更新 + 从默认分支建特性分支

```bash
cd /Users/shaoziyuan/Documents/战略大盘/safety-platform
git fetch origin
DEF=$(git symbolic-ref --short refs/remotes/origin/HEAD | sed 's#origin/##')   # 探测默认分支(main/master)
echo "default branch = $DEF"
git checkout "$DEF" && git pull
...
```bash
DST=/Users/shaoziyuan/Documents/战略大盘/safety-platform/services/policy-pipeline
mkdir -p "$DST"
...
```bash
cd /Users/shaoziyuan/Documents/战略大盘/safety-platform/services/policy-pipeline
...
**→ Claude 审 MR diff;平台团队 review + 合。合并后进 Phase B。**

---

## Phase B · VPS cutover(用户亲手 · server-ops 红线 · Codex 给命令不替跑)

**前置**:Phase A 的 MR 已合进 monorepo 默认分支。
