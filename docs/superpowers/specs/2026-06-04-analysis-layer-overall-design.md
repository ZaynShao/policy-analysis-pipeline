...
## 10.5 下游纳入门(Lever B):让"低估"不致"埋没"〔2026-06-05 补〕

②-B 的 judge 对"地方政策被低估(underrate)"有已知盲区(实测必须抓 ~67%)。若 ③/④ 仅按 `重要性` 筛重点,被低估的政策会被静默丢弃(漏报)。Lever B 在**消费时**用并集门兜底——**不改已写的 business_view**(无需 re-apply):

下游纳入(深档 / ③-C 候选池 / ④ 呈现)= 满足任一即纳入:

- `重要性 ≥ 3`,或
- `region.level ∈ {国, 省}`,或
- **关系枢纽**:被 ≥1 篇 `cites_basis` 或 `derives_from` 指向(别人拿它当依据)。

第三条**独立于 importance 分**,故 underrate 单独不足以漏掉一篇政策;漏判被收窄到"低估 **且** 地方 **且** 无人引用"——低风险。这是**全局规则、非 pid 补丁**;且是**消费时门、不触发 business_view re-apply**。它收掉 ②-B"低分飘过"留下的潜在漏报挂账。
